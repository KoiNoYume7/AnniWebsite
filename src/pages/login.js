// OAuth config — replace with your actual client IDs and backend URLs
// Your backend at /api/auth/* handles the OAuth flows
const OAUTH = {
  github: {
  ...
  discord: {
    ...
    icon: `<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M20.317 4.37a19.791 19.791 0 00-4.885-1.515.074.074 0 00-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 00-5.487 0 12.64 12.64 0 00-.617-1.25.077.077 0 00-.079-.037A19.736 19.736 0 003.677 4.37a.07.07 0 00-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 00.031.057 19.9 19.9 0 005.993 3.03.078.078 0 00.084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 00-.041-.106 13.107 13.107 0 01-1.872-.892.077.077 0 01-.008-.128 10.2 10.2 0 00.372-.292.074.074 0 01.077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 01.078.01c.12.098.246.198.373.292a.077.077 0 01-.006.127 12.299 12.299 0 01-1.873.892.077.077 0 00-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 00.084.028 19.839 19.839 0 006.002-3.03.077.077 0 00.032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 00-.031-.03z"/></svg>`,
    color: 'oauth-discord',
...
export async function renderLogin(root) {
    ...
    <section style="min-height:80vh;display:flex;align-items:center;justify-content:center;padding:60px 20px">
      <div style="width:100%;max-width:420px">
        <div class="card reveal" style="padding:44px">
          <div style="text-align:center;margin-bottom:36px">
            ...
            <p style="color:var(--muted);font-size:0.88rem;line-height:1.6">
              Sign in to access the status dashboard and private tools.
              Only authorized accounts can get in.
            </p>
          </div>

          <div id="oauth-buttons">
            ${Object.entries(OAUTH).map(([key, p]) => `
              <button class="oauth-btn ${p.color}" onclick="startOAuth('${key}')">
                ${p.icon}
                ${p.label}
              </button>
            `).join('')}
        ...
        <p style="text-align:center;margin-top:20px;font-size:0.8rem;color:var(--muted)">
          Not a developer? <button style="background:none;border:none;color:var(--accent);cursor:pointer;font-size:inherit;font-family:inherit" onclick="navigate('')">Go back home →</button>
        </p>
      </div>
    </section>
  `
  ...
  window.startOAuth = (provider) => {
    const btns = document.getElementById('oauth-buttons')
    if (btns) btns.innerHTML = `<div style="text-align:center;padding:20px;color:var(--muted);font-family:var(--font-mono);font-size:0.82rem">Connecting to ${provider}…</div>`

    // In production: redirect to /api/auth/${provider}
    // For now, simulate auth with demo user
    setTimeout(() => {
      const demoUser = {
        name: 'KoiNoYume7',
        provider,
      ...
      sessionStorage.setItem('anni_user', JSON.stringify(demoUser))

      // Update nav
      const navBtn = document.getElementById('navLoginBtn')
      if (navBtn) {
        navBtn.textContent = demoUser.name
        navBtn.onclick = () => navigate('status')
      }

      import('../main.js').then(m => m.showToast(`✅ Signed in via ${provider}`))
      navigate('status')
    }, 1200)
  }
}
    ...
    <section style="min-height:60vh;display:flex;align-items:center;justify-content:center;padding:60px 20px">
      <div style="width:100%;max-width:420px">
        <div class="card reveal" style="padding:44px;text-align:center">
          <div style="font-size:2.4rem;margin-bottom:14px">✅</div>
          <h2 style="font-family:var(--font-head);font-size:1.4rem;font-weight:800;margin-bottom:8px">
            You're in, ${user.name}
          </h2>
          <p style="color:var(--muted);font-size:0.88rem;margin-bottom:28px">
